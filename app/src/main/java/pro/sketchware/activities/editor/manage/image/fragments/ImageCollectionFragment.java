package pro.sketchware.activities.editor.manage.image.fragments;
...
public class ImageCollectionFragment extends BaseFragment implements View.OnClickListener {
    ...
    private final ActivityResultLauncher<Intent> openImageImportDetails = registerForActivityResult(new ActivityResultContracts.StartActivityForResult(), result -> {
        ...
        if (result.getResultCode() == Activity.RESULT_OK && data != null && isAdded()) {
            ...
            if (!newImportedImages.isEmpty()) {
                ((ManageImageActivity) requireActivity()).getProjectImagesFragment().importImages(newImportedImages);
                ((ManageImageActivity) requireActivity()).setCurrentPage(0);
            }
        }
    });
    ...
    public void refreshData() {
        collectionImages = ImageCollectionManager.getInstance().getResources();
    ...
    public void importImages() {
        ...
        for (ProjectResourceBean image : collectionImages) {
            if (image.isSelected) {
                selectedCollections.add(new ProjectResourceBean(ProjectResourceBean.PROJECT_RES_TYPE_FILE, image.resName, SketchwarePaths.getCollectionPath() + File.separator + "image" + File.separator + "data" + File.separator + image.resFullName));
            }
        }
    ...
    private class Adapter extends RecyclerView.Adapter<Adapter.ViewHolder> {
        ...
        public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
            ...
            Glide.with(requireActivity())
                    .asBitmap()
                    .load(SketchwarePaths.getCollectionPath() + File.separator + "image" + File.separator + "data" + File.separator + image.resFullName)
                    .centerCrop()
