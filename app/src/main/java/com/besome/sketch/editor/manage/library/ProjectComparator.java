...
public class ProjectComparator implements Comparator<HashMap<String, Object>> {
    ...
    public int compare(HashMap<String, Object> first, HashMap<String, Object> second) {
        ...
        if (Objects.equals(pinned_scid, MapValueHelper.c(first, "sc_id"))) {
            return -1;
        } else if (Objects.equals(pinned_scid, MapValueHelper.c(second, "sc_id"))) {
            return 1;
        }

        if ((sortBy & SORT_BY_ID) == SORT_BY_ID) {
            return Integer.compare(
                    Integer.parseInt(MapValueHelper.c(first, "sc_id")),
                    Integer.parseInt(MapValueHelper.c(second, "sc_id"))) * (isSortOrderAscending ? 1 : -1);
        } else if ((sortBy & SORT_BY_NAME) == SORT_BY_NAME) {
            return MapValueHelper.c(first, "my_ws_name").compareTo(MapValueHelper.c(second, "my_ws_name")) * (isSortOrderAscending ? 1 : -1);
        } else {
            return Integer.compare(
                    Integer.parseInt(MapValueHelper.c(first, "sc_id")),
                    Integer.parseInt(MapValueHelper.c(second, "sc_id"))) * -1;
        }
    }
}
