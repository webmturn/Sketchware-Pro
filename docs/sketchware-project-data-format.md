...
3. **推送到设备**（`project` 推到 `mysc/list/{id}/`；其余项目数据至少推到 `data/{id}/`。如果该项目已有旧的 `bak/{id}/` 备份，需同步更新或删除它以避免恢复旧内容）
...
> - 建议使用完整的 ViewBean 模板；省略字段通常会回落到构造器默认值，但不要用 `""` 代替应为 `null` 的字段
...
> 将以上项目文件按各自位置推送：`project` 到 `mysc/list/700/project`，其余 5 个项目数据文件至少推送到 `data/700/`，即可在 Sketchware 中编译运行。若该项目已存在不同的 `bak/700/` 备份，请同步更新或删除旧备份。
...
```json
{"opCode":"intentSetAction","parameters":["intent1","ACTION_VIEW"],"spec":"%m.intent setAction %m.intentAction"}
...
```json
{"opCode":"setVisible","parameters":["panel_list","GONE"],"spec":"%m.view setVisible %m.visible","type":" "}
{"opCode":"setVisible","parameters":["panel_edit","VISIBLE"],"spec":"%m.view setVisible %m.visible","type":" "}
```
...
如果想手动模拟一个底部导航栏，可以用 LinearLayout + 可点击 TextView 组合实现：
...
{"opCode":"setVisible","parameters":["linear_tab3_panel","GONE"],"spec":"%m.view setVisible %m.visible","type":" "}
{"opCode":"addSourceDirectly","parameters":["tv_tab1.setTextColor(0xFFFFFFFF); tv_tab1.setTypeface(null, android.graphics.Typeface.BOLD); tv_tab2.setTextColor(0xFFA8A8A8); tv_tab2.setTypeface(null, android.graphics.Typeface.NORMAL); tv_tab3.setTextColor(0xFFA8A8A8); tv_tab3.setTypeface(null, android.graphics.Typeface.NORMAL);"],"type":" "}
```

> 这里以 `tv_tab1` 为例；`tv_tab2` / `tv_tab3` 也需要各自声明 `onClick` 事件，并把可见面板与文字样式切换到对应 tab。

### for-each 列表遍历

遍历字符串列表中的每个元素（`forEachStr` 是 C 形块，循环体变量通过 `%s.inputOnly` 指定）：
...
// 积木块
{"id":"1","nextBlock":2,"opCode":"addListStr","parameters":["Alice","names"],"spec":"add %s to %m.listStr","subStack1":-1,"subStack2":-1,"type":" "}
{"id":"2","nextBlock":-1,"opCode":"forEachStr","parameters":["_item","names"],"spec":"for each item %s.inputOnly in %m.listStr","subStack1":10,"subStack2":-1,"type":"c"}
// subStack1 内的块（每次循环执行）
{"id":"10","nextBlock":-1,"opCode":"addSourceDirectly","parameters":["Log.d(\"TAG\", _item);"],"spec":"add source directly %s.inputOnly","subStack1":-1,"subStack2":-1,"type":" "}
```
...
> **注意**: `forEachStr` / `forEachNum` / `forEachMap` 的循环变量是**局部变量**（不在 `_var` 节中声明），
> 在循环体内只能通过 `addSourceDirectly` 引用。三种块分别生成 `String`、`double`、`HashMap<String, Object>` 类型的循环变量。

### if-else-if 多条件分支

使用 `ifElseChain`（E 形块）+ `elseIf`/`elseBlock`（C 形块，放在 subStack2 中）实现扁平的多条件分支：

```json
// ifElseChain: subStack1=if体, subStack2=elseIf/elseBlock链
{"id":"1","nextBlock":-1,"opCode":"ifElseChain","parameters":["@100"],"spec":"if %b","subStack1":10,"subStack2":20,"type":"e"}
{"id":"100","nextBlock":-1,"opCode":"asdBoolean","parameters":["score >= 90"],"spec":"boolean %s.inputOnly","type":"b"}
// subStack1: if体
{"id":"10","nextBlock":-1,"opCode":"addSourceDirectly","parameters":["grade = \"A\";"],"type":" "}
// subStack2: elseIf → elseBlock 通过 nextBlock 链接
{"id":"20","nextBlock":30,"opCode":"elseIf","parameters":["@101"],"spec":"else if %b","subStack1":21,"subStack2":-1,"type":"c"}
{"id":"101","nextBlock":-1,"opCode":"asdBoolean","parameters":["score >= 80"],"type":"b"}
{"id":"21","nextBlock":-1,"opCode":"addSourceDirectly","parameters":["grade = \"B\";"],"type":" "}
{"id":"30","nextBlock":-1,"opCode":"elseBlock","parameters":[],"spec":"else","subStack1":31,"subStack2":-1,"type":"c"}
{"id":"31","nextBlock":-1,"opCode":"addSourceDirectly","parameters":["grade = \"F\";"],"type":" "}
```
...
> **注意**: `elseIf` 和 `elseBlock` 只能放在 `ifElseChain` 的 subStack2 中，通过 `nextBlock` 链接多个。
> `elseBlock` 应放在链的末尾（`nextBlock=-1`）。与嵌套 `ifElse` 相比，此方式生成扁平的 `if/else if/else` 代码，无额外缩进。
...
> 父容器的 `weightSum` 可留空或设为 `0`；生成器不会写出该属性，布局会按子视图 `weight` 的默认规则分配空间。
...
{
    "id": "et1",
    "inject": "android:scrollbars=\"vertical\" android:inputType=\"textMultiLine\"",
    "type": 5,
    ...
}
```
...
View Editor 现在提供 **Import XML** 功能，可把标准 Android 布局 XML 转成当前项目的 `ViewBean` 列表。

**入口**：View Editor 菜单 → `Import XML`

**界面输入**：
- **Layout XML**：必填，待导入的布局 XML
- **dimens.xml**：可选，用于解析 `@dimen/...` 引用

**导入行为**：
- 支持常见 Android 原生标签以及当前已注册的扩展视图
- `MaterialCardView` 会按 `CardView` 处理
- 未识别的标签会按 `LinearLayout` 导入，并把原标签名写入 `convert`
- `android:id` 会被解析为 `id`，自动去掉 `@+id/` 或 `@id/`
- 如果导入 ID 与当前页面已有 ID 冲突，会自动改名为 `xxx_2`、`xxx_3` 这种形式
- 冲突改名后，导入批次内的父子引用也会同步更新
- `tools:listitem` 会被单独处理：对 `ListView` / `GridView` / `Spinner` / `RecyclerView` / `ViewPager`，如果值是 `@layout/...`，会写入 `customView`；否则保留到 `inject`
- 未识别属性会写入 `inject`，原样保留到生成的 XML 中
- `@dimen/...`：优先从 `dimens.xml` 解析；解析不到时保留到 `inject`，并给出 warning

**结果反馈**：
- 空 XML 会提示 `No views found in XML`
- 解析异常会提示 `XML parse error: ...`
- 成功后会提示导入数量；如果有 warning，也会附加警告数量

**实现位置**：
- `pro.sketchware.core.XmlLayoutParser`：负责 XML → `ViewBean` 解析
- `pro.sketchware.core.ViewEditorFragment`：负责导入对话框、冲突处理和写回项目数据
...
> **重要**: 打开项目时默认从 `data/` 加载；只有当 `bak/` 中存在**非空且与 `data/` 不同**的有效备份文件时，才会恢复对应备份。手动改文件时至少更新 `data/`；如果还保留着旧的 `bak/` 备份，也应同步更新或删除。
...
> `bak/{id}/` 目录可以为空或不存在。手动创建新项目时，只写 `data/` 目录也能正常冷加载。
> 只有当 `bak/` 中存在非空且与 `data/` 不同的备份文件时，加载器才会优先使用它；如果你是在覆盖一个已有项目，建议同步更新或删除旧 `bak/{id}/`。
...
> **注意**: `project` 文件（在 `mysc/list/{id}/` 下）**不加密**，始终为明文 JSON。只有 `data/` 和 `bak/` 目录下的项目数据文件才按同样规则加密；最小项目通常是 `file` / `library` / `logic` / `resource` / `view` 这 5 个文件。
...
| 26 | Notification |
| 27 | FragmentAdapter |
| 28 | PhoneAuth (FirebasePhoneAuth) |
| 29 | SQLiteDatabase |
| 30 | **Cloud Message** (FirebaseCloudMessage) |
| 31 | Google Login (FirebaseGoogleLogin) |
| 35 | Camera Legacy (Videos) |
| 36 | AsyncTask |
...
| SwipeRefreshLayout | `onSwipeRefreshLayout` | 下拉刷新 |
...
| VideoView | `onCompletion` | 播放完成 |
| VideoView | `onPrepared` | 准备完成 |
| VideoView | `onError` | 播放错误 |
...
| PhoneAuth (FirebasePhoneAuth) | `onVerificationCompleted` | 验证完成 |
| PhoneAuth (FirebasePhoneAuth) | `onVerificationFailed` | 验证失败 |
| PhoneAuth (FirebasePhoneAuth) | `onCodeSent` | 验证码已发送 |
| Cloud Message (FirebaseCloudMessage) | `onCompleteRegister` | FCM注册完成 |
| Google Login (FirebaseGoogleLogin) | `onAccountPicker` | 账号选择 |
| Google Login (FirebaseGoogleLogin) | `onAccountPickerCancelled` | 账号选择取消 |
...
| Firebase Auth | `signInWithPhoneAuthComplete` | 手机认证登录完成 |
| Firebase Auth | `onGoogleSignIn` | Google 登录完成 |
| SQLiteDatabase | `onSQLiteError` | SQLite 错误 |

> **注意**: 上表中的 Firebase Auth 子事件（`onUpdateProfileComplete` 等）属于 Firebase Auth 组件 (type=12)，
> 不是独立的 FirebasePhoneAuth/Google Login 组件事件。同样，`signInWithPhoneAuthComplete` 和 `onGoogleSignIn` 也属于 Firebase Auth 组件。
...
| 非字符串变量引用 | 直接写变量名 | `"count"`, `"isLoggedIn"` | 适用于 `%d` / `%b` 等；`%s` 需使用返回值块或 `%s.inputOnly` |
...
// 块2: 直接代码放入 edittext 的值（此处示例按未启用 ViewBinding 的写法）
{"id":"2","nextBlock":5,"opCode":"addSourceDirectly",
 "parameters":["map.put(\"message\", edittext1.getText().toString());"],
 "spec":"add source directly %s.inputOnly",
 "subStack1":-1,"subStack2":-1,"type":" ","color":-11899692}
...
// 块8: 在 onDataChange 回调中执行（被 subStack1 引用，直接插入 Java 表达式）
{"id":"8","nextBlock":-1,"opCode":"addSourceDirectly",
 "parameters":["textview1.setText(String.valueOf(fbData));"],
 "spec":"add source directly %s.inputOnly",
 "subStack1":-1,"subStack2":-1,"type":" ","color":-11899692}
```
...
```json
// 直接使用 addSourceDirectly 引用事件回调的局部变量 _childValue
{"id":"11","nextBlock":-1,"opCode":"addSourceDirectly",
 "parameters":["textview1.setText(String.valueOf(_childValue));"],
 "spec":"add source directly %s.inputOnly",
 "subStack1":-1,"subStack2":-1,"type":" ","color":-11899692}
```
...
> `type` 字段表示返回类型（`"d"`=数字、`"s"`=字符串、`"l"`=列表、`"m"`=Map）。例如：
> ```json
> // 获取 onBindCustomView 中的 position（数字类型）
> {"opCode":"getArg","parameters":[],"spec":"position","type":"d"}
> // 获取 onBindCustomView 中的 data（Map 类型）
> {"opCode":"getArg","parameters":[],"spec":"data","type":"m","typeName":"Map"}
> ```
...
| `strToMap` | JSON→Map | [`%s` JSON字符串, `%m.varMap` Map] |
| `mapToStr` | Map→JSON | [`%m.varMap` Map] |
...
| `strToListMap` | JSON→ListMap | [`%s` JSON字符串, `%m.listMap` ListMap] |
| `listMapToStr` | ListMap→JSON | [`%m.listMap` ListMap] |
| `reverseList` | 反转列表 | [List] |
| `shuffleList` | 随机排列列表 | [List] |
| `sortList` | 排序字符串列表 | [StringList] |
| `sortListnum` | 排序数字列表 | [NumberList] |
| `swapInList` | 交换列表元素 | [List, 位置1, 位置2] |
| `getMapAtPosListmap` | 获取ListMap中Map对象 | [索引, ListMap] |
| `setMapAtPosListmap` | 设置ListMap中Map对象 | [Map, 索引, ListMap] |
| `setAtPosListstr` | 设置StringList中元素 | [值, 索引, StringList] |
| `setAtPosListnum` | 设置NumberList中元素 | [值, 索引, NumberList] |
| `sortListmap` | 排序ListMap | [ListMap, 键, isNumber, isAscending] |
| `deleteMapFromListmap` | 从ListMap删除Map | [Map, ListMap] |
| `mapContainValue` | Map包含值 | [Map, 值] |
| `GsonListTojsonString` | List→JSON(Gson) | [List] |
| `GsonStringToListString` | JSON→StringList(Gson) | [JSON, StringList] |
| `GsonStringToListNumber` | JSON→NumberList(Gson) | [JSON, NumberList] |
...
| `whileLoop` | while循环 | [条件] | subStack1=循环体 |
| `forEachStr` | 遍历字符串列表 | [变量名, StringList] | subStack1=循环体, 变量名通过 `%s.inputOnly` 指定 |
| `forEachNum` | 遍历数字列表 | [变量名, NumberList] | subStack1=循环体 |
| `forEachMap` | 遍历Map列表 | [变量名, MapList] | subStack1=循环体 |
| `break` | 跳出循环 | — | type="f" |
| `continue` | 跳过本次迭代 | — | type="f" |
| `ifElseChain` | 多条件分支 | [条件] | subStack1=if体, subStack2=elseIf/elseBlock链 (type="e") |
| `elseIf` | else-if 分支 | [条件] | subStack1=分支体, 放在 ifElseChain 的 subStack2 中 |
| `elseBlock` | else 分支 | — | subStack1=分支体, 放在 ifElseChain 的 subStack2 末尾 |
| `switchStr` | 字符串switch | [字符串] | subStack1=case链 |
| `switchNum` | 数字switch | [数字] | subStack1=case链, 生成 `switch((int)x)` |
| `caseStr` | 字符串case | [值] | subStack1=分支体, 自动生成 `break` |
| `caseNum` | 数字case | [值] | subStack1=分支体, 自动生成 `break` |
| `defaultSwitch` | default分支 | — | subStack1=分支体, 自动生成 `break` |
| `tryCatch` | 异常捕获 | — | subStack1=try体, subStack2=catch体 (type="e") |
| `getExceptionMessage` | 获取异常信息 | — | 返回 `e.getMessage()`（在 catch 中使用, type="s"） |
| `returnString` | 返回字符串 | [字符串] | type="f", 生成 `return (x);` |
| `returnNumber` | 返回数字 | [数字] | type="f" |
| `returnBoolean` | 返回布尔 | [布尔] | type="f" |
| `returnMap` | 返回Map | [Map] | type=" " |
| `returnListStr` | 返回字符串列表 | [ListStr] | type=" " |
| `returnListMap` | 返回Map列表 | [ListMap] | type=" " |
| `returnView` | 返回视图 | [View] | type=" " |
| `returnTitle` | 返回标题（FragmentAdapter） | [标题] | type="f" |
| `returnFragment` | 返回Fragment（FragmentAdapter） | [Activity] | type="f" |
| `repeatKnownNum` | 带变量的重复(递增) | [次数, 变量名] | subStack1=循环体, `for (int i=0; i<n; i++)` |
| `RepeatKnownNumDescending` | 带变量的重复(递减) | [次数, 变量名] | subStack1=循环体, `for (int i=n-1; i>-1; i--)` |
| `forLoopIncrease` | for循环(自增) | [变量, 初始值, 条件] | subStack1=循环体 |
| `catchBlock` | catch 块 | [异常变量名] | 放在 tryCatch 的 subStack2 中 |
| `finallyBlock` | finally 块 | — | 放在 tryCatch 的 subStack2 末尾 |
| `caseStrAnd` | 合并多个字符串case | [值] | type=" ", 连续的caseStrAnd合并到下一个caseStr |
| `caseNumAnd` | 合并多个数字case | [值] | type=" " |
...
| `true` `false` | 布尔值 | — |
| `random` | 随机数 | [最小, 最大] |
| `ternaryString` | 三元运算(字符串) | [条件, 真值, 假值] |
| `ternaryNumber` | 三元运算(数字) | [条件, 真值, 假值] |
| `instanceOfOperator` | instanceof 判断 | [对象, 类名] |
| `isEmpty` | 判断空字符串 | [字符串] |
...
| `stringMatches` | 正则匹配 | [字符串, 正则] |
| `stringSubSingle` | 截取(单参) | [字符串, 起始] |
| `stringSplitToList` | 分割到列表 | [字符串, 正则, StringList] |
| `trim` | 去空格 | [字符串] |
| `toUpperCase` | 转大写 | [字符串] |
| `toLowerCase` | 转小写 | [字符串] |
...
| `strParseInteger` | 字符串→整数 | [字符串] |
| `toHashCode` | 哈希码 | [对象] |
| `reverse` | 反转字符串 | [字符串] |
| `html` | HTML→文本 | [HTML字符串] |
| `currentTime` | 当前时间戳 | — |
...
| `calendarViewSetMaxDate` | 设置最大日期 | [CalendarView, 时间戳] |
| `calnedarViewSetMaxDate` | 设置最大日期（⚠️ 兼容历史 typo） | [CalendarView, 时间戳] |
...
| `asdBoolean` | 直接插入布尔表达式 | [代码字符串] |
| `asdNumber` | 直接插入数字表达式 | [代码字符串] |
| `asdString` | 直接插入字符串表达式 | [代码字符串] |
| `customToast` | 自定义Toast | [文本, 文字颜色, 字号, 背景色, 圆角, 重力] |
| `customToastWithIcon` | 自定义Toast(带图标) | [文本, 文字颜色, 字号, 背景色, 圆角, 重力, 图标] |
| `hideKeyboard` | 隐藏键盘 | — |
| `showKeyboard` | 显示键盘 | — |
| `LightStatusBar` | 亮色状态栏 | — |
| `changeStatebarColour` | 更改状态栏颜色 | [Activity, 颜色] |
| `isConnected` | 检查网络连接 | — |
| `getResString` | 获取字符串资源 | [资源名] |
| `startService` | 启动Service | [Activity] |
| `stopService` | 停止Service | [Activity] |
| `sendBroadcast` | 发送广播 | [Action] |
| `customImport` | 添加import语句 | [类名] |
| `customImport2` | 添加import(选择器) | [import项] |

> **`addSourceDirectly` 中引用视图**: 未启用 ViewBinding 时，view 文件中定义的视图会在 `initialize()` 方法中生成字段，可直接写 `edittext1.setText("...")`。
> 若启用 ViewBinding（`enable_viewbinding:"true"`），`addSourceDirectly` 中必须显式写 `binding.edittext1...`，不会像标准块那样自动转换。
...
| `intentHasExtra` | 判断是否包含Extra | [键] |
| `intentSetType` | 设置MIME类型 | [Intent, 类型] |
| `intentRemoveExtra` | 删除Extra | [Intent, 键] |
| `launchApp` | 获取指定包名的启动Intent | [Intent, 包名] |
| `startActivityWithChooser` | 使用Chooser启动Activity | [Intent, 提示文本] |
| `finishAffinity` | 关闭当前任务栈 | — |
...
| `cropBitmapFileFromCenter` | 中心裁剪 | [路径, 目标, 宽, 高] ⚠️ |
...
> ⚠️ `cropBitmapFileFromCenter` 的代码生成会交换第 3、4 个参数：`FileUtil.cropBitmapFileFromCenter(path, dest, params[3], params[2])`，
> 即 UI 上标注的"宽"实际传给了 Java 方法的 `h` 参数、"高"传给了 `w` 参数。这是源码中的已有行为。
...
| `requestnetworkUploadFile` | 上传文件 | [组件, URL, FileKey, FilePath, Tag] |
...
| `mapViewZoomIn` | 放大 | [MapView] |
| `mapViewZoomOut` | 缩小 | [MapView] |
...
| `mapViewSetMarkerColor` | 设置标记颜色 | [MapView, ID, 颜色, Alpha] |
...
#### Google Login (FirebaseGoogleLogin, type=31)

| opCode | 说明 | 参数 |
|--------|------|------|
| `googleSignInInit` | 初始化（设置 Web Client ID） | [组件, ClientID] |
| `googleSignInLaunch` | 启动登录流程 | [组件] |
| `googleSignOut` | 登出 | [组件] |

#### PhoneAuth (FirebasePhoneAuth, type=28)

| opCode | 说明 | 参数 |
|--------|------|------|
| `phoneAuthSendCode` | 发送验证码 | [组件, 手机号] |
| `phoneAuthResendCode` | 重新发送验证码 | [组件, 手机号] |
| `phoneAuthSignIn` | 使用手机凭证登录 | [FirebaseAuth, verificationId, code] |

> **注意**: `phoneAuthSignIn` 的第一个参数是 `%m.firebaseauth`（Firebase Auth 组件），不是 PhoneAuth 组件本身。

#### Cloud Message (FirebaseCloudMessage, type=30)

| opCode | 说明 | 参数 |
|--------|------|------|
| `fcmGetToken` | 获取 FCM Token | [组件] |
| `fcmSubscribeTopic` | 订阅主题 | [组件, 主题名] |
| `fcmUnsubscribeTopic` | 取消订阅主题 | [组件, 主题名] |

> 这三个组件同时也是**事件驱动**的，有各自的事件回调（见事件表）。
> 代码生成器会自动创建所需的监听器和回调代码。

#### SQLiteDatabase (type=29)

| opCode | 说明 | 参数 | type |
|--------|------|------|------|
| `sqliteOpen` | 打开/创建数据库 | [组件, 数据库名] | `" "` |
| `sqliteClose` | 关闭数据库 | [组件] | `" "` |
| `sqliteExecSQL` | 执行SQL（无返回值） | [组件, SQL] | `" "` |
| `sqliteRawQuery` | 执行查询（返回Cursor） | [组件, SQL] | `" "` |
| `sqliteMoveToFirst` | 移到第一行 | [组件] | `b` |
| `sqliteMoveToNext` | 移到下一行 | [组件] | `b` |
| `sqliteIsAfterLast` | 是否越过最后一行 | [组件] | `b` |
| `sqliteGetString` | 获取字符串列值 | [组件, 列名] | `s` |
| `sqliteGetNumber` | 获取数字列值 | [组件, 列名] | `d` |
| `sqliteGetCount` | 获取行数 | [组件] | `d` |
| `sqliteCloseCursor` | 关闭Cursor | [组件] | `" "` |
| `sqliteIsOpen` | 数据库是否打开 | [组件] | `b` |
| `sqliteCursorIsNull` | Cursor是否为null | [组件] | `b` |
| `sqliteForEachRow` | 遍历每一行 | [组件] | `c` |
| `sqliteBeginTransaction` | 开始事务 | [组件] | `" "` |
| `sqliteSetTransactionSuccessful` | 标记事务成功 | [组件] | `" "` |
| `sqliteEndTransaction` | 结束事务 | [组件] | `" "` |
| `sqliteEnableWAL` | 启用WAL模式 | [组件] | `" "` |

> **注意**: SQLite 组件会生成双字段：`SQLiteDatabase {id}` 和 `Cursor _{id}_cursor`。
> 在 `onDestroy` 中自动关闭数据库连接。`sqliteForEachRow` 是 C 形块，内部代码在每行循环中执行。

#### OTPView

| opCode | 说明 | 参数 | type |
|--------|------|------|------|
| `otpViewSetFieldCount` | 设置字段数 | [OTPView, 数量] | `" "` |
| `otpViewSetOTPText` | 设置OTP文本 | [OTPView, 文本] | `" "` |
| `otpViewGetOTPText` | 获取OTP文本 | [OTPView] | `s` |
| `otpViewSetOTPListener` | 设置OTP输入完成监听 | [OTPView] | `c` |

#### SignInButton

| opCode | 说明 | 参数 |
|--------|------|------|
| `signInButtonSetColorScheme` | 设置配色方案 | [SignInButton, 方案] |
| `signInButtonSetSize` | 设置大小 | [SignInButton, 大小] |

#### AdView（横幅广告）

| opCode | 说明 | 参数 |
|--------|------|------|
| `bannerAdViewLoadAd` | 加载横幅广告 | [AdView] |

#### 视图工具块

| opCode | 说明 | 参数 | type |
|--------|------|------|------|
| `checkViewVisibility` | 检查视图可见性 | [View, 可见性] | `b` |
| `viewGetChildAt` | 获取子视图 | [View, 索引] | `v` |
| `webviewGetProgress` | 获取WebView加载进度 | [WebView] | `d` |

#### Menu 块

> 仅在 `onCreateOptionsMenu` 事件中可用。

| opCode | 说明 | 参数 | type |
|--------|------|------|------|
| `menuItemSetVisible` | 设置菜单项可见性 | [MenuItem, true/false] | `" "` |
| `menuItemSetEnabled` | 设置菜单项启用状态 | [MenuItem, true/false] | `" "` |
| `menuFindItem` | 查找菜单项 | [ID] | `v` |

#### File 自定义变量块

> 需要声明 `File` 类型的自定义变量（变量类型 5，格式 `File myFile`）才会显示。

| opCode | 说明 | 参数 | type |
|--------|------|------|------|
| `fileCanExecute` | 是否可执行 | [File] | `b` |
| `fileCanRead` | 是否可读 | [File] | `b` |
| `fileCanWrite` | 是否可写 | [File] | `b` |
| `fileGetName` | 获取文件名 | [File] | `s` |
| `fileGetParent` | 获取父路径 | [File] | `s` |
| `fileGetPath` | 获取完整路径 | [File] | `s` |
| `fileIsHidden` | 是否隐藏 | [File] | `b` |

#### 列表扩展

| opCode | 说明 | 参数 |
|--------|------|------|
| `listAddAll` | 将一个列表全部添加到另一个列表 | [StringList, List] |

#### FileUtil 扩展

| opCode | 说明 | 参数 |
|--------|------|------|
| `fileutilcopydir` | 复制目录 | [源路径, 目标路径] |
| `fileContainsData` | SharedPrefs 包含键 | [File组件, 键名] |

#### 命令块（编译时处理）

> 这些块在编译时由特殊逻辑处理，不直接生成 Java 代码。

| opCode | 说明 | 参数 |
|--------|------|------|
| `addPermission` | 添加权限 | [权限名] |
| `removePermission` | 移除权限 | [权限名] |
| `addCustomVariable` | 添加自定义变量声明 | [声明语句] |
| `addInitializer` | 添加初始化代码 | [代码] |

> 以上未标注来源的扩展块均定义在 `ExtraBlocks.java` 中。

#### BlocksHandler 扩展块

> 以下块定义在 `BlocksHandler.java` 中，部分需要开启 `SETTING_SHOW_BUILT_IN_BLOCKS` 后才在面板显示。

**EditText 扩展**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `EditTextdiableSuggestion` | 禁用建议 | [EditText] |
| `EditTextLines` | 设置行数 | [EditText, 行数] |
| `EditTextSingleLine` | 设置单行 | [EditText, true/false] |
| `EditTextShowError` | 显示错误 | [EditText, 错误文本] |
| `EditTextSelectAll` | 全选文本 | [EditText] |
| `EditTextSetSelection` | 设置选区 | [EditText, 起始, 结束] |
| `EditTextSetMaxLines` | 设置最大行数 | [EditText, 行数] |
| `EdittextGetselectionStart` | 获取选区起始 | [EditText] |
| `EdittextGetselectionEnd` | 获取选区结束 | [EditText] |
| `setTextSize` | 设置文字大小 | [TextView, 大小] |

**View 操作扩展**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `performClick` | 模拟点击 | [View] |
| `getHeight` | 获取高度 | [View] |
| `getWidth` | 获取宽度 | [View] |
| `removeView` | 移除子视图 | [父View, 子View] |
| `removeViews` | 移除所有子视图 | [View] |
| `addView` | 添加子视图 | [父View, 子View] |
| `addViews` | 添加子视图到索引 | [父View, 子View, 索引] |
| `setGravity` | 设置重力 | [View, 垂直, 水平] |
| `setElevation` | 设置阴影 | [View, 高度] |
| `setColorFilterView` | 设置背景滤镜 | [View, 颜色, PorterDuff模式] |
| `setCornerRadiusView` | 设置圆角背景 | [View, 半径, 颜色] |
| `setGradientBackground` | 设置渐变背景 | [View, 颜色1, 颜色2] |
| `setStrokeView` | 设置边框 | [View, 宽度, 边框色, 背景色] |
| `setRadiusAndStrokeView` | 设置圆角+边框 | [View, 半径, 边框宽, 边框色, 背景色] |
| `setBgDrawable` | 设置Drawable背景 | [View, Drawable资源名] |
| `showSnackbar` | 显示Snackbar | [View, 文本, 按钮文本] (subStack1=点击逻辑) |
| `viewOnLongClick` | 长按监听 | [View] (subStack1=逻辑) |
| `viewOnTouch` | 触摸监听 | [View] (subStack1=逻辑) |

**ImageView 扩展**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `setImageIdentifier` | 按名称设置图片 | [ImageView, 资源名字符串] |
| `setImageCustomRes` | 设置资源图片 | [ImageView, 资源] |
| `imageCrop` | 裁剪图片 | [路径, RequestCode] |

**ListView 扩展**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `refreshingList` | 刷新视图 | [ListView] |
| `ListViewAddHeader` | 添加头部 | [ListView, View, 数据, 可选] |
| `ListViewAddFooter` | 添加底部 | [ListView, View, 数据, 可选] |
| `listViewRemoveHeader` | 移除头部 | [ListView, View] |
| `listViewRemoveFooter` | 移除底部 | [ListView, View] |
| `listViewSetSelection` | 设置选中位置 | [ListView, 位置] |
| `getLastVisiblePosition` | 获取末尾可见位置 | [ListView] |
| `listscrollparam` | 滚动参数常量 | [参数名] |
| `listSetSelector` | 设置选择器颜色 | [ListView, 颜色] |
| `listSetTranscriptMode` | 设置滚动模式 | [ListView, 模式] |
| `listSetStackFromBottom` | 从底部堆叠 | [ListView, true/false] |
...
| opCode | 说明 | 参数 |
|--------|------|------|
| `checkboxOnChecked` | 选中状态变化监听 | [CheckBox] (subStack1=逻辑) |
| `checkboxIsChecked` | 获取isChecked(回调内) | — |

**VideoView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `videoviewSetVideoUri` | 设置视频URI | [VideoView, URI] |
| `videoviewStart` | 开始播放 | [VideoView] |
| `videoviewPause` | 暂停 | [VideoView] |
| `videoviewStop` | 停止播放 | [VideoView] |
| `videoviewIsPlaying` | 是否播放中 | [VideoView] |
| `videoviewCanPause` | 是否可暂停 | [VideoView] |
| `videoviewCanSeekForward` | 是否可快进 | [VideoView] |
| `videoviewCanSeekBackward` | 是否可快退 | [VideoView] |
| `videoviewGetDuration` | 获取时长 | [VideoView] |
| `videoviewGetCurrentPosition` | 获取当前位置 | [VideoView] |

**RatingBar**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `getRating` | 获取评分 | [RatingBar] |
| `setRating` | 设置评分 | [RatingBar, 值] |
| `setNumStars` | 设置星数 | [RatingBar, 数量] |
| `setStepSize` | 设置步长 | [RatingBar, 步长] |

**TimePicker**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `timepickerSetIs24Hour` | 设置24小时制 | [TimePicker, true/false] |
| `timepickerSetCurrentHour` | 设置当前小时(旧API) | [TimePicker, 小时] |
| `timepickerSetCurrentMinute` | 设置当前分钟(旧API) | [TimePicker, 分钟] |
| `timepickerSetHour` | 设置小时(新API) | [TimePicker, 小时] |
| `timepickerSetMinute` | 设置分钟(新API) | [TimePicker, 分钟] |

**GridView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `gridSetCustomViewData` | 设置自定义适配器 | [GridView, MapList] |
| `gridSetNumColumns` | 设置列数 | [GridView, 列数] |
| `gridSetColumnWidth` | 设置列宽 | [GridView, 宽度] |
| `gridSetVerticalSpacing` | 设置纵向间距 | [GridView, 间距] |
| `gridSetHorizontalSpacing` | 设置横向间距 | [GridView, 间距] |
| `gridSetStretchMode` | 设置拉伸模式 | [GridView, 模式] |

**AutoCompleteTextView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `autoComSetData` | 设置建议数据 | [ACTV, StringList] |
| `setThreshold` | 设置触发阈值 | [MACTV, 字符数] |
| `setTokenizer` | 设置逗号分隔 | [MACTV] |
| `multiAutoComSetData` | 设置建议数据 | [MACTV, StringList] |

**CardView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `setCardBackgroundColor` | 设置背景色 | [CardView, 颜色] |
| `setCardRadius` | 设置圆角 | [CardView, 半径] |
| `setCardElevation` | 设置阴影 | [CardView, 高度] |
| `setPreventCornerOverlap` | 防止圆角重叠 | [CardView, true/false] |
| `setUseCompatPadding` | 使用兼容内边距 | [CardView, true/false] |

**TabLayout**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `addTab` | 添加标签页 | [TabLayout, 标题] |
| `setupWithViewPager` | 关联ViewPager | [TabLayout, ViewPager] |
| `setInlineLabel` | 设置内联标签 | [TabLayout, true/false] |
| `setTabTextColors` | 设置文字颜色 | [TabLayout, 普通色, 选中色] |
| `setTabRippleColor` | 设置水波纹颜色 | [TabLayout, 颜色] |
| `setSelectedTabIndicatorColor` | 设置指示器颜色 | [TabLayout, 颜色] |
| `setSelectedTabIndicatorHeight` | 设置指示器高度 | [TabLayout, 高度] |

**ViewPager**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `pagerSetFragmentAdapter` | 设置FragmentAdapter | [ViewPager, Adapter, Tab数] |
| `pagerSetCustomViewData` | 设置自定义适配器 | [ViewPager, MapList] |
| `pagerGetOffscreenPageLimit` | 获取离屏页数 | [ViewPager] |
| `pagerSetOffscreenPageLimit` | 设置离屏页数 | [ViewPager, 数量] |
| `pagerGetCurrentItem` | 获取当前页 | [ViewPager] |
| `pagerSetCurrentItem` | 设置当前页 | [ViewPager, 位置] |
| `ViewPagerNotifyOnDtatChange` | 通知数据变化 | [ViewPager] |

**RecyclerView 扩展**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `setRecyclerViewLayoutParams` | 设置布局参数 | [宽, 高] |
| `recyclerSetCustomViewData` | 设置适配器 | [RecyclerView, MapList] |
| `recyclerSetLayoutManager` | 设置纵向布局管理器 | [RecyclerView] |
| `recyclerSetLayoutManagerHorizontal` | 设置横向布局管理器 | [RecyclerView] |
| `recyclerSetHasFixedSize` | 设置固定尺寸 | [RecyclerView, true/false] |
| `recyclerSmoothScrollToPosition` | 平滑滚动 | [RecyclerView, 位置] |
| `recyclerScrollToPositionWithOffset` | 带偏移滚动 | [RecyclerView, 位置, 偏移] |

**SwipeRefreshLayout**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `onSwipeRefreshLayout` | 下拉刷新监听 | [SRL] (subStack1=刷新逻辑) |
| `setRefreshing` | 设置刷新状态 | [SRL, true/false] |

**BottomNavigationView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `bottomMenuAddItem` | 添加菜单项 | [BottomNav, ID, 标题, 图标] |
...
| `fabIcon` | 设置FAB图标 | [资源名] |
| `fabSize` | 设置FAB尺寸 | [尺寸] |
| `fabVisibility` | 设置FAB可见性 | [show/hide] |

**Dialog 扩展**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `Dialog SetIcon` | 设置对话框图标 | [Dialog, 资源名] |

**Menu 扩展**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `menuInflater` | 从XML加载菜单 | [菜单文件] |
| `menuAddItem` | 添加菜单项 | [ID, 标题] |
| `menuAddMenuItem` | 添加带图标菜单项 | [MenuItem, ID, 标题, 图标, showAsAction] |
| `menuAddSubmenu` | 添加子菜单 | [Submenu, 标题] (subStack1=子菜单项) |
| `submenuAddItem` | 子菜单添加项 | [Submenu, ID, 标题] |

**File 扩展**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `getAssetFile` | 获取Asset文件流 | [InputStream, 路径] |
| `renameFile` | 重命名文件 | [原路径, 新路径] |
| `copyAssetFile` | 读取InputStream为字符串 | [InputStream] |

**CodeView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `codeviewSetCode` | 设置代码 | [CodeView, 代码] |
| `codeviewSetTheme` | 设置主题 | [CodeView, 主题] |
| `codeviewSetLanguage` | 设置语言 | [CodeView, 语言] |
| `codeviewApply` | 应用设置 | [CodeView] |

**LottieAnimationView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `lottieSetAnimationFromAsset` | 从Asset加载动画 | [Lottie, 路径] |
| `lottieSetAnimationFromJson` | 从JSON加载动画 | [Lottie, JSON] |
| `lottieSetAnimationFromUrl` | 从URL加载动画 | [Lottie, URL] |
| `lottieSetRepeatCount` | 设置重复次数 | [Lottie, 次数] |
| `lottieSetSpeed` | 设置播放速度 | [Lottie, 速度] |

**YouTubePlayerView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `YTPVLifecycle` | 绑定生命周期 | [YouTubeView] |
| `YTPVSetListener` | 设置播放器监听 | [YouTubeView, VideoID] (subStack1=就绪逻辑) |

**BadgeView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `getBadgeCount` | 获取徽章数 | [BadgeView] |
| `setBadgeNumber` | 设置徽章数字 | [BadgeView, 数字] |
| `setBadgeString` | 设置徽章文本 | [BadgeView, 文本] |
| `setBadgeBackground` | 设置徽章背景色 | [BadgeView, 颜色] |
| `setBadgeTextColor` | 设置徽章文字色 | [BadgeView, 颜色] |
| `setBadgeTextSize` | 设置徽章字号 | [BadgeView, 大小] |

**PatternLockView**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `patternToString` | 图案→字符串 | [PatternView, List] |
| `patternToMD5` | 图案→MD5 | [PatternView, List] |
| `patternToSha1` | 图案→SHA1 | [PatternView, List] |
| `patternSetDotCount` | 设置点数 | [PatternView, 数量] |
| `patternSetNormalStateColor` | 设置正常颜色 | [PatternView, 颜色] |
| `patternSetCorrectStateColor` | 设置正确颜色 | [PatternView, 颜色] |
| `patternSetWrongStateColor` | 设置错误颜色 | [PatternView, 颜色] |
| `patternSetViewMode` | 设置视图模式 | [PatternView, 模式] |
| `patternLockClear` | 清除图案 | [PatternView] |

**TextInputLayout**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `tilSetBoxBgColor` | 设置输入框背景色 | [TIL, 颜色] |
| `tilSetBoxStrokeColor` | 设置边框颜色 | [TIL, 颜色] |
| `tilSetBoxBgMode` | 设置背景模式 | [TIL, 模式] |
| `tilSetBoxCornerRadii` | 设置圆角 | [TIL, 左上, 右上, 左下, 右下] |
| `tilSetError` | 设置错误提示 | [TIL, 文本] |
| `tilSetErrorEnabled` | 启用错误提示 | [TIL, true/false] |
| `tilSetCounterEnabled` | 启用字数计数 | [TIL, true/false] |
| `tilSetCounterMaxLength` | 设置最大字数 | [TIL, 长度] |
| `tilGetCounterMaxLength` | 获取最大字数 | [TIL] |

**WaveSideBar (BubbleLayout)**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `setCustomLetter` | 设置自定义字母 | [SideBar, 字母数组] |
| `setBubbleColor` | 设置气泡颜色 | [View, 颜色] |
| `setBubbleStrokeColor` | 设置气泡边框色 | [View, 颜色] |
| `setBubbleStrokeWidth` | 设置气泡边框宽 | [View, 宽度] |
| `setBubbleCornerRadius` | 设置气泡圆角 | [View, 半径] |
| `setBubbleArrowHeight` | 设置箭头高度 | [View, 高度] |
| `setBubbleArrowWidth` | 设置箭头宽度 | [View, 宽度] |
| `setBubbleArrowPosition` | 设置箭头位置 | [View, 位置] |

**InterstitialAd 新 API**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `interstitialAdLoad` | 加载插页广告 | [组件, Activity] |
| `interstitialAdShow` | 显示插页广告 | [组件, Activity] |
| `interstitialAdIsLoaded` | 是否已加载 | [组件] |

**Command Blocks**:

| opCode | 说明 | 参数 |
|--------|------|------|
| `CommandBlockJava` | Java命令块 | [引用, 距离, 前端, 后端, 命令] (subStack1=代码) |
| `CommandBlockXML` | XML命令块 | [引用, 距离, 前端, 后端, 命令, XML名] (subStack1=代码) |
...
@MainActivity.java_DB_onChildAdded
{"color":-11899692,"id":"2","nextBlock":-1,"opCode":"addSourceDirectly","parameters":["textview1.setText(String.valueOf(_childValue));"],"spec":"add source directly %s.inputOnly","subStack1":-1,"subStack2":-1,"type":" ","typeName":""}
```
...
| `%m.intentAction` | Intent Action | `ACTION_VIEW`, `ACTION_SEND`, `ACTION_CALL` 等 |
| `%m.intentFlags` | Intent Flags | `SINGLE_TOP`, `CLEAR_TOP` 等 |
...
| `onSwipeRefreshLayoutListener` | `onSwipeRefreshLayout` |
...
| `sqliteErrorHandler` | `onSQLiteError` |
...
| `onSQLiteError` | `_errorMessage` (String) |
...
| `onPreExecute` | — |
| `onBindCustomView` | `_position` (int), `_data` (ArrayList&lt;HashMap&gt;, 适配器类字段), 各自定义视图组件引用（如 `custom_item_tv_title`） |
| `onChildMoved` | — |
...
| `onCompletion` / `onPrepared` | `_mediaPlayer` (MediaPlayer) |
| `onError` | `_mediaPlayer` (MediaPlayer), `_what` (int), `_extra` (int) |
...
| 27 | FragmentAdapter | — | — | — |
| 28 | PhoneAuth (FirebasePhoneAuth) | — | — | — |
| 29 | SQLiteDatabase | `SQLiteDatabase` | `Cursor` | 自动在 `onDestroy` 关闭 |
| 30 | **Cloud Message** (FirebaseCloudMessage) | — | — | — |
| 31 | Google Login (FirebaseGoogleLogin) | — | — | — |
| 35 | Camera Legacy (Videos) | — | — | — |
| 36 | AsyncTask | — | — | — |

> **注意**: 对于 Firebase DB（type=6），`param1` 是 `DatabaseReference` 的路径。生成代码示例：`_firebase.getReference("test")`。
> 对于 Firebase Storage（type=14），`param1` 是 `StorageReference` 的路径。生成代码示例：`_firebase_storage.getReference("uploads/images")`。
...
核心文件至少要写入 `data/{id}/`。如果该项目下还保留着旧的、与 `data/` 不同的 `bak/{id}/` 备份，也要同步更新或删除，否则 Sketchware 可能恢复旧数据。
...
ViewBean 字段并非“全部必填”：缺失字段通常会回落到构造器默认值（Gson 反序列化行为）。
但为了兼容性与可维护性，仍建议以完整模板为基准，只修改必要字段。

> **注意**: `resTextColor`、`resHintColor`、`backgroundResColor` 这类资源字段如提供，必须是 `null` 或有效资源名，**不可为空字符串 `""`**。
...
`intentSetFlags` 的参数（菜单值，生成代码会自动补上 `Intent.FLAG_ACTIVITY_` 前缀）：

| 值 | 说明 |
|----|------|
| `SINGLE_TOP` | 如已在栈顶则不重新创建 |
| `CLEAR_TOP` | 清除目标之上的 Activity |
...
> 以下表格由 `BlockSpecRegistry.java` + `BlocksHandler.java` + `ExtraBlocks.java` + `strings.xml` 提取生成。
> 覆盖所有 `BlockSpecRegistry` / `ExtraBlocks` 的 opCode 以及 `BlocksHandler` 中的扩展块。
...
| `asdBoolean` | `boolean %s.inputOnly` | `b` |
| `asdNumber` | `number %s.inputOnly` | `d` |
| `asdString` | `string %s.inputOnly` | `s` |
...
| `mapContainValue` | `%m.varMap contain value %s` | `b` |
...
| `reverseList` | `reverse %m.list` | `" "` |
| `shuffleList` | `shuffle %m.list` | `" "` |
| `sortList` | `sort %m.listStr` | `" "` |
| `sortListnum` | `sort %m.listInt` | `" "` |
| `swapInList` | `swap %m.list position %d with %d` | `" "` |
| `setAtPosListstr` | `set %s at %d of %m.listStr` | `" "` |
| `setAtPosListnum` | `set %d at %d of %m.listInt` | `" "` |
| `setMapAtPosListmap` | `set %m.varMap at %d of %m.listMap` | `" "` |
| `getMapAtPosListmap` | `get Map at %d of %m.listMap` | `a` |

**Map 扩展操作**:

| opCode | spec | type |
|--------|------|------|
| `hashmapGetNumber` | `%m.varMap get number key %s` | `d` |
| `hashmapPutNumber` | `%m.varMap put key %s value int %d` | `" "` |
| `hashmapPutNumber2` | `%m.varMap put key %s value double %d` | `" "` |
| `hashmapGetBoolean` | `%m.varMap get boolean key %s` | `b` |
| `hashmapPutBoolean` | `%m.varMap put key %s value %b` | `" "` |
| `hashmapGetMap` | `%m.varMap get Map key %s` | `a` |
| `hashmapPutMap` | `%m.varMap put key %s value %m.varMap` | `" "` |
| `hashmapListstr` | `%m.varMap get List String key %s` | `l` |
| `hashmapPutListstr` | `%m.varMap put key %s value %m.listStr` | `" "` |
| `hashmapGetListmap` | `%m.varMap get List Map key %s` | `l` |
| `hashmapPutListmap` | `%m.varMap put key %s value %m.listMap` | `" "` |
...
| `forever` | `forever` | `c` |
| `whileLoop` | `while %b` | `c` |
| `repeatKnownNum` | `repeat %d: %s.inputOnly ++` | `c` |
| `RepeatKnownNumDescending` | `repeat %d: %s.inputOnly --` | `c` |
| `forEachStr` | `for each item %s.inputOnly in %m.listStr` | `c` |
| `forEachNum` | `for each item %s.inputOnly in %m.listInt` | `c` |
| `forEachMap` | `for each item %s.inputOnly in %m.listMap` | `c` |
| `ifElseChain` | `if %b` (spec2: `else if / else`) | `e` |
| `elseIf` | `else if %b` | `c` |
| `elseBlock` | `else` | `c` |
| `switchStr` | `switch %s` | `c` |
| `switchNum` | `switch %d` | `c` |
| `caseStr` | `case %s` | `c` |
| `caseStrAnd` | `case %s and` | `" "` |
| `caseNum` | `case %d` | `c` |
| `caseNumAnd` | `case %d and` | `" "` |
| `defaultSwitch` | `default` | `c` |
| `tryCatch` | `try` (spec2: `catch / finally`) | `e` |
| `getExceptionMessage` | `exception message` | `s` |
| `break` | `stop` | `f` |
| `continue` | `continue` | `f` |
| `returnString` | `return %s` | `f` |
| `returnNumber` | `return %d` | `f` |
| `returnBoolean` | `return %b` | `f` |
| `returnMap` | `return %m.varMap` | `f` |
| `returnListStr` | `return %m.listStr` | `f` |
| `returnListMap` | `return %m.listMap` | `f` |
| `returnView` | `return %m.view` | `f` |
| `ternaryString` | `%b ? %s : %s` | `s` |
| `ternaryNumber` | `%b ? %d : %d` | `d` |
| `instanceOfOperator` | `%s instanceOf %s` | `b` |
| `isEmpty` | `%s isEmpty` | `b` |
| `true` | `true` | `b` |
| `false` | `false` | `b` |
...
| `stringReplaceFirst` | `%s replace first RegExp %s with %s` | `s` |
| `stringReplaceAll` | `%s replace all RegExp %s with %s` | `s` |
| `stringMatches` | `%s matches RegExp %s` | `b` |
| `stringSubSingle` | `%s subString %d` | `s` |
| `reverse` | `reverse %s` | `s` |
| `html` | `html %s` | `s` |
...
| `strParseInteger` | `parse int %s` | `d` |
| `toHashCode` | `toHashCode %s` | `d` |
...
| `GsonListTojsonString` | `%m.list to JSON String` | `s` |
| `GsonStringToListString` | `JSON %s to %m.listStr` | `" "` |
| `GsonStringToListNumber` | `JSON %s to %m.listInt` | `" "` |
| `stringSplitToList` | `split %s RegExp %s into %m.listStr` | `" "` |
...
| `calendarViewSetMaxDate` | `%m.calendarview setMaxDate %d ms` | `" "` |
...
| `finishActivity` | `Finish Activity` | `f` |
| `intentHasExtra` | `Activity hasExtra key %s` | `b` |
| `intentSetType` | `%m.intent setType %s` | `" "` |
| `intentRemoveExtra` | `%m.intent removeExtra key %s` | `" "` |
| `launchApp` | `%m.intent set app package %s` | `" "` |
| `startActivityWithChooser` | `StartActivity %m.intent with Chooser %s` | `" "` |
| `finishAffinity` | `Finish Affinity` | `f` |
...
| `requestnetworkUploadFile` | `%m.requestnetwork upload file to url %s file key %s file path %s with tag %s` | `" "` |
...
| opCode | spec | type |
|--------|------|------|
| `notifCreateChannel` | `%m.notification createChannel id %s name %s importance %m.notifImportance` | `" "` |
| `notifSetChannel` | `%m.notification setChannel %s` | `" "` |
| `notifSetTitle` | `%m.notification setTitle %s` | `" "` |
| `notifSetContent` | `%m.notification setContent %s` | `" "` |
| `notifSetSmallIcon` | `%m.notification setSmallIcon %m.resource` | `" "` |
| `notifSetAutoCancel` | `%m.notification setAutoCancel %b` | `" "` |
| `notifSetPriority` | `%m.notification setPriority %m.notifPriority` | `" "` |
| `notifSetClickIntent` | `%m.notification setClickIntent %m.intent` | `" "` |
| `notifShow` | `%m.notification show id %d` | `" "` |
| `notifCancel` | `%m.notification cancel id %d` | `" "` |

**SQLiteDatabase (type=29)**:

| opCode | spec | type |
|--------|------|------|
| `sqliteOpen` | `%m.sqlite open database %s` | `" "` |
| `sqliteClose` | `%m.sqlite close` | `" "` |
| `sqliteExecSQL` | `%m.sqlite execSQL %s` | `" "` |
| `sqliteRawQuery` | `%m.sqlite rawQuery %s` | `" "` |
| `sqliteMoveToFirst` | `%m.sqlite moveToFirst` | `b` |
| `sqliteMoveToNext` | `%m.sqlite moveToNext` | `b` |
| `sqliteIsAfterLast` | `%m.sqlite isAfterLast` | `b` |
| `sqliteGetString` | `%m.sqlite getString column %s` | `s` |
| `sqliteGetNumber` | `%m.sqlite getNumber column %s` | `d` |
| `sqliteGetCount` | `%m.sqlite getCount` | `d` |
| `sqliteCloseCursor` | `%m.sqlite closeCursor` | `" "` |
| `sqliteIsOpen` | `%m.sqlite isOpen` | `b` |
| `sqliteCursorIsNull` | `%m.sqlite cursorIsNull` | `b` |
| `sqliteForEachRow` | `%m.sqlite forEachRow` | `c` |
| `sqliteBeginTransaction` | `%m.sqlite beginTransaction` | `" "` |
| `sqliteSetTransactionSuccessful` | `%m.sqlite setTransactionSuccessful` | `" "` |
| `sqliteEndTransaction` | `%m.sqlite endTransaction` | `" "` |
| `sqliteEnableWAL` | `%m.sqlite enableWAL` | `" "` |

**Google Login (FirebaseGoogleLogin, type=31)**:

| opCode | spec | type |
|--------|------|------|
| `googleSignInInit` | `%m.googlelogin initialize with web client id %s` | `" "` |
| `googleSignInLaunch` | `%m.googlelogin launch sign in` | `" "` |
| `googleSignOut` | `%m.googlelogin sign out` | `" "` |

**PhoneAuth (FirebasePhoneAuth, type=28)**:

| opCode | spec | type |
|--------|------|------|
| `phoneAuthSendCode` | `%m.phoneauth send verification code to %s` | `" "` |
| `phoneAuthResendCode` | `%m.phoneauth resend verification code to %s` | `" "` |
| `phoneAuthSignIn` | `%m.firebaseauth sign in with phone credential verificationId %s code %s` | `" "` |

**Cloud Message (FirebaseCloudMessage, type=30)**:

| opCode | spec | type |
|--------|------|------|
| `fcmGetToken` | `%m.cloudmessage get token` | `" "` |
| `fcmSubscribeTopic` | `%m.cloudmessage subscribe to topic %s` | `" "` |
| `fcmUnsubscribeTopic` | `%m.cloudmessage unsubscribe from topic %s` | `" "` |

**OTPView**:

| opCode | spec | type |
|--------|------|------|
| `otpViewSetFieldCount` | `%m.otpview setFieldCount %d` | `" "` |
| `otpViewSetOTPText` | `%m.otpview setOTPText %s` | `" "` |
| `otpViewGetOTPText` | `%m.otpview getOTPText` | `s` |
| `otpViewSetOTPListener` | `%m.otpview onOTPEntered -> _otp` | `c` |

**SignInButton**:

| opCode | spec | type |
|--------|------|------|
| `signInButtonSetColorScheme` | `%m.signinbutton setColorScheme %m.SignButtonColor` | `" "` |
| `signInButtonSetSize` | `%m.signinbutton setSize %m.SignButtonSize` | `" "` |

**AdView（横幅广告扩展）**:

| opCode | spec | type |
|--------|------|------|
| `bannerAdViewLoadAd` | `%m.adview load` | `" "` |

**视图工具块**:

| opCode | spec | type |
|--------|------|------|
| `checkViewVisibility` | `visibility of %m.view equals %m.visible` | `b` |
| `viewGetChildAt` | `%m.view getChildAt %d` | `v` |
| `webviewGetProgress` | `%m.webview getProgress` | `d` |

**Menu 块**:

| opCode | spec | type |
|--------|------|------|
| `menuItemSetVisible` | `%m.MenuItem setVisible %b` | `" "` |
| `menuItemSetEnabled` | `%m.MenuItem setEnabled %b` | `" "` |
| `menuFindItem` | `MenuItem findItem %s.inputOnly` | `v` |

**File 自定义变量块**:

| opCode | spec | type |
|--------|------|------|
| `fileCanExecute` | `%m.File canExecute` | `b` |
| `fileCanRead` | `%m.File canRead` | `b` |
| `fileCanWrite` | `%m.File canWrite` | `b` |
| `fileGetName` | `%m.File getName` | `s` |
| `fileGetParent` | `%m.File getParent` | `s` |
| `fileGetPath` | `%m.File getPath` | `s` |
| `fileIsHidden` | `%m.File isHidden` | `b` |

**列表/文件扩展**:

| opCode | spec | type |
|--------|------|------|
| `listAddAll` | `%m.listStr addAll from %m.list` | `" "` |
| `fileutilcopydir` | `copy dir path %s to path %s` | `" "` |
| `fileContainsData` | `%m.file contains %s` | `b` |

**命令块**:

| opCode | spec | type |
|--------|------|------|
| `addPermission` | `Permission Command Block: add %m.Permission` | `" "` |
| `removePermission` | `Permission Command Block: remove %m.Permission` | `" "` |
| `addCustomVariable` | `Custom Variable Block: add variable %s` | `" "` |
| `addInitializer` | `Initializer Block: add initializer %s` | `" "` |
| `CommandBlockJava` | `Java Command Block: reference %s distance %d frontend %d backend %d command %m.Command` | `c` |
| `CommandBlockXML` | `XML Command Block: reference %s distance %d frontend %d backend %d command %m.Command xml name %s.inputOnly` | `c` |

**控制流扩展（仅 BlocksHandler 新增）**:

| opCode | spec | type |
|--------|------|------|
| `forLoopIncrease` | `for %m.varInt = %d; %b; %m.varInt++` | `c` |
| `catchBlock` | `catch %s.inputOnly` | `c` |
| `finallyBlock` | `finally` | `c` |
| `returnTitle` | `return Title %s` | `f` |
| `returnFragment` | `return Fragment %m.activity` | `f` |

**List 扩展（仅 BlocksHandler 新增）**:

| opCode | spec | type |
|--------|------|------|
| `sortListmap` | `sort %m.listMap key %s isNumber %b isAscending %b` | `" "` |
| `deleteMapFromListmap` | `delete %m.varMap of %m.listMap` | `a` |

**导入块**:

| opCode | spec | type |
|--------|------|------|
| `customImport` | `import %s.import` | `" "` |
| `customImport2` | `import %m.import` | `" "` |
...
| `changeStatebarColour` | `%m.activity set statebar color %m.color` | `" "` |
| `isConnected` | `isConnected` | `" "` |
| `getClipboard` | `get clipboard text` | `s` |
| `getResString` | `get String from %m.ResString` | `s` |
| `startService` | `startService %m.activity` | `" "` |
| `stopService` | `stopService %m.activity` | `" "` |
| `sendBroadcast` | `sendBroadcast %s` | `" "` |

**Toast 扩展**:

| opCode | spec | type |
|--------|------|------|
| `customToast` | `CustomToast %s textColor %m.color textSize %d bgColor %m.color cornerRadius %d gravity %m.gravity_t` | `" "` |
| `customToastWithIcon` | `CustomToastWithIcon %s textColor %m.color textSize %d bgColor %m.color cornerRadius %d gravity %m.gravity_t Icon %m.resource` | `" "` |

**EditText 扩展**:

| opCode | spec | type |
|--------|------|------|
| `EditTextdiableSuggestion` | `%m.edittext disable suggestions` | `" "` |
| `EditTextLines` | `%m.edittext set lines %d` | `" "` |
| `EditTextSingleLine` | `%m.edittext singleLine? %b` | `" "` |
| `EditTextShowError` | `%m.edittext show error %s` | `" "` |
| `EditTextSelectAll` | `%m.edittext select all text` | `" "` |
| `EditTextSetSelection` | `%m.edittext set selection start %d end %d` | `" "` |
| `EditTextSetMaxLines` | `%m.edittext set max lines %d` | `" "` |
| `EdittextGetselectionStart` | `%m.edittext get selection start` | `d` |
| `EdittextGetselectionEnd` | `%m.edittext get selection end` | `d` |
| `setTextSize` | `%m.textview setTextSize %d` | `" "` |

**View 操作扩展**:

| opCode | spec | type |
|--------|------|------|
| `performClick` | `%m.view performClick` | `" "` |
| `getHeight` | `%m.view getHeight` | `d` |
| `getWidth` | `%m.view getWidth` | `d` |
| `removeView` | `%m.view removeView %m.view` | `" "` |
| `removeViews` | `%m.view removeAllViews` | `" "` |
| `addView` | `%m.view addView %m.view` | `" "` |
| `addViews` | `%m.view addView %m.view index %d` | `" "` |
| `setGravity` | `%m.view setGravity %m.gravity_v %m.gravity_h` | `" "` |
| `setElevation` | `%m.view setElevation %d` | `" "` |
| `setColorFilterView` | `%m.view setColorFilter %m.color with %m.porterduff` | `" "` |
| `setCornerRadiusView` | `%m.view setCornerRadius %d color %m.color` | `" "` |
| `setGradientBackground` | `%m.view setGradientBackground %m.color and %m.color` | `" "` |
| `setStrokeView` | `%m.view setStroke %d strokeColor %m.color bgColor %m.color` | `" "` |
| `setRadiusAndStrokeView` | `%m.view setCornerRadius %d stroke %d strokeColor %m.color bgColor %m.color` | `" "` |
| `setBgDrawable` | `%m.view setBackgroundDrawable %m.drawable` | `" "` |
| `showSnackbar` | `%m.view showSnackbar text %s actionText %s onClick` | `c` |
| `viewOnLongClick` | `When %m.view long clicked` | `c` |
| `viewOnTouch` | `When %m.view touched` | `c` |

**ImageView 扩展**:

| opCode | spec | type |
|--------|------|------|
| `setImageIdentifier` | `%m.imageview set image by name %s` | `" "` |
| `setImageCustomRes` | `%m.imageview setImage %m.image` | `" "` |
| `imageCrop` | `CropImageView fromFilePath %s RequestCode %d` | `" "` |

**ListView 扩展**:

| opCode | spec | type |
|--------|------|------|
| `refreshingList` | `%m.listview invalidate views` | `" "` |
| `ListViewAddHeader` | `%m.listview add Header view %m.view data %s selectable? %b` | `" "` |
| `ListViewAddFooter` | `%m.listview add Footer view %m.view data %s selectable? %b` | `" "` |
| `listViewRemoveHeader` | `%m.listview remove Header %m.view` | `" "` |
| `listViewRemoveFooter` | `%m.listview remove Footer %m.view` | `" "` |
| `listViewSetSelection` | `%m.listview set selection %d` | `" "` |
| `getLastVisiblePosition` | `%m.listview getLastVisiblePosition` | `d` |
| `listscrollparam` | `%m.listscrollparam` | `d` |
| `listSetSelector` | `%m.listview setSelector %m.color` | `" "` |
| `listSetTranscriptMode` | `%m.listview setTranscriptMode %m.transcriptmode` | `" "` |
| `listSetStackFromBottom` | `%m.listview setStackFromBottom %b` | `" "` |
| `notifyDataSetChanged` | `RefreshData` | `" "` |

**CheckBox 扩展**:

| opCode | spec | type |
|--------|------|------|
| `checkboxOnChecked` | `When %m.checkbox checked` | `c` |
| `checkboxIsChecked` | `isChecked` | `b` |

**RatingBar**:

| opCode | spec | type |
|--------|------|------|
| `getRating` | `%m.ratingbar getRating` | `d` |
| `setRating` | `%m.ratingbar setRating%d` | `" "` |
| `setNumStars` | `%m.ratingbar setNumStars %d` | `" "` |
| `setStepSize` | `%m.ratingbar setStepSize %d` | `" "` |

**VideoView**:

| opCode | spec | type |
|--------|------|------|
| `videoviewSetVideoUri` | `%m.videoview setVideoUri %s` | `" "` |
| `videoviewStart` | `%m.videoview start` | `" "` |
| `videoviewPause` | `%m.videoview pause` | `" "` |
| `videoviewStop` | `%m.videoview stopPlayback` | `" "` |
| `videoviewIsPlaying` | `%m.videoview isPlaying` | `b` |
| `videoviewCanPause` | `%m.videoview canPause` | `b` |
| `videoviewCanSeekForward` | `%m.videoview canSeekForward` | `b` |
| `videoviewCanSeekBackward` | `%m.videoview canSeekBackward` | `b` |
| `videoviewGetDuration` | `%m.videoview getDuration` | `d` |
| `videoviewGetCurrentPosition` | `%m.videoview getCurrentPosition` | `d` |

**TimePicker**:

| opCode | spec | type |
|--------|------|------|
| `timepickerSetIs24Hour` | `%m.timepicker setIs24Hour %b` | `" "` |
| `timepickerSetCurrentHour` | `%m.timepicker setCurrentHour %d` | `" "` |
| `timepickerSetCurrentMinute` | `%m.timepicker setCurrentMinute%d` | `" "` |
| `timepickerSetHour` | `%m.timepicker setHour %d` | `" "` |
| `timepickerSetMinute` | `%m.timepicker setMinute%d` | `" "` |

**GridView**:

| opCode | spec | type |
|--------|------|------|
| `gridSetCustomViewData` | `%m.gridview setGridCustomViewData %m.listMap` | `" "` |
| `gridSetNumColumns` | `%m.gridview setNumColumns %d` | `" "` |
| `gridSetColumnWidth` | `%m.gridview setColumnWidth %d` | `" "` |
| `gridSetVerticalSpacing` | `%m.gridview setVerticalSpacing %d` | `" "` |
| `gridSetHorizontalSpacing` | `%m.gridview setHorizontalSpacing %d` | `" "` |
| `gridSetStretchMode` | `%m.gridview setStretchMode %m.gridstretchmode` | `" "` |

**AutoCompleteTextView**:

| opCode | spec | type |
|--------|------|------|
| `autoComSetData` | `%m.actv setListData %m.listStr` | `" "` |
| `setThreshold` | `%m.mactv setThreshold %d` | `" "` |
| `setTokenizer` | `%m.mactv CommaTokenizer` | `" "` |
| `multiAutoComSetData` | `%m.mactv setListData %m.listStr` | `" "` |

**CardView**:

| opCode | spec | type |
|--------|------|------|
| `setCardBackgroundColor` | `%m.cardview setCardBackgroundColor %m.color` | `" "` |
| `setCardRadius` | `%m.cardview setCornerRadius %d` | `" "` |
| `setCardElevation` | `%m.cardview setCardElevation %d` | `" "` |
| `setPreventCornerOverlap` | `%m.cardview setPreventCornerOverlap %b` | `" "` |
| `setUseCompatPadding` | `%m.cardview setUseCompatPadding %b` | `" "` |

**TabLayout**:

| opCode | spec | type |
|--------|------|------|
| `addTab` | `%m.tablayout addTabTitle %s` | `" "` |
| `setupWithViewPager` | `%m.tablayout setupWithViewPager %m.viewpager` | `" "` |
| `setInlineLabel` | `%m.tablayout setInlineLabel %b` | `" "` |
| `setTabTextColors` | `%m.tablayout setTabTextColors Normal %m.color Selected %m.color` | `" "` |
| `setTabRippleColor` | `%m.tablayout setTabRippleColor %m.color` | `" "` |
| `setSelectedTabIndicatorColor` | `%m.tablayout setSelectedTabIndicatorColor %m.color` | `" "` |
| `setSelectedTabIndicatorHeight` | `%m.tablayout setSelectedTabIndicatorHeight %d` | `" "` |

**ViewPager**:

| opCode | spec | type |
|--------|------|------|
| `pagerSetFragmentAdapter` | `%m.viewpager setFragmentAdapter %m.fragmentAdapter TabCount %d` | `" "` |
| `pagerSetCustomViewData` | `%m.viewpager setPagerCustomViewData %m.listMap` | `" "` |
| `pagerGetOffscreenPageLimit` | `%m.viewpager getOffscreenPageLimit` | `d` |
| `pagerSetOffscreenPageLimit` | `%m.viewpager setOffscreenPageLimit %d` | `" "` |
| `pagerGetCurrentItem` | `%m.viewpager getCurrentItem` | `d` |
| `pagerSetCurrentItem` | `%m.viewpager setCurrentItem %d` | `" "` |
| `ViewPagerNotifyOnDtatChange` | `%m.viewpager notifyDataSetChanged` | `" "` |
| `pagerscrollparam` | `%m.pagerscrollparam` | `d` |

**RecyclerView**:

| opCode | spec | type |
|--------|------|------|
| `setRecyclerViewLayoutParams` | `set RecyclerViewLayoutParams width %m.LayoutParam height %m.LayoutParam` | `" "` |
| `recyclerSetCustomViewData` | `%m.recyclerview setRecyclerCustomViewData %m.listMap` | `" "` |
| `recyclerSetLayoutManager` | `%m.recyclerview setLayoutManager` | `" "` |
| `recyclerSetLayoutManagerHorizontal` | `%m.recyclerview set Horizontal LayoutManager` | `" "` |
| `recyclerSetHasFixedSize` | `%m.recyclerview setHasFixedSize %b` | `" "` |
| `recyclerSmoothScrollToPosition` | `%m.recyclerview smoothScrollToPosition %d` | `" "` |
| `recyclerScrollToPositionWithOffset` | `%m.recyclerview scrollToPosition %d offset %d ` | `" "` |
| `recyclerscrollparam` | `%m.recyclerscrollparam` | `d` |

**SwipeRefreshLayout**:

| opCode | spec | type |
|--------|------|------|
| `onSwipeRefreshLayout` | `When %m.swiperefreshlayout refreshed` | `c` |
| `setRefreshing` | `%m.swiperefreshlayout setRefreshing %b` | `" "` |

**BottomNavigationView**:

| opCode | spec | type |
|--------|------|------|
| `bottomMenuAddItem` | `%m.bottomnavigation add item id %d title %s icon %m.resource` | `" "` |

**FAB 扩展**:

| opCode | spec | type |
|--------|------|------|
| `fabIcon` | `FAB set icon %m.resource` | `" "` |
| `fabSize` | `FAB setSize %m.fabsize` | `" "` |
| `fabVisibility` | `FAB setVisibility %m.fabvisible` | `" "` |

**Dialog 扩展**:

| opCode | spec | type |
|--------|------|------|
| `Dialog SetIcon` | `%m.dialog setIcon %m.resource_bg` | `" "` |

**Menu 扩展**:

| opCode | spec | type |
|--------|------|------|
| `menuInflater` | `Menu get menu from file %m.menu` | `" "` |
| `menuAddItem` | `Menu add id %d title %s` | `" "` |
| `menuAddMenuItem` | `%m.menuitem add id %d title %s icon %m.resource showAsAction %m.menuaction` | `" "` |
| `menuAddSubmenu` | `Menu add %m.submenu title %s;` | `c` |
| `submenuAddItem` | `%m.submenu add id %d title %s` | `" "` |

**File 扩展**:

| opCode | spec | type |
|--------|------|------|
| `getAssetFile` | `%m.inputstream getFileFromAsset path %s` | `" "` |
| `renameFile` | `rename file path %s to %s` | `" "` |
| `copyAssetFile` | `%m.inputstream to String` | `s` |

**CodeView**:

| opCode | spec | type |
|--------|------|------|
| `codeviewSetCode` | `%m.codeview setCode %s` | `" "` |
| `codeviewSetTheme` | `%m.codeview setTheme %m.cv_theme` | `" "` |
| `codeviewSetLanguage` | `%m.codeview setLanguage %m.cv_language` | `" "` |
| `codeviewApply` | `%m.codeview apply` | `" "` |

**LottieAnimationView**:

| opCode | spec | type |
|--------|------|------|
| `lottieSetAnimationFromAsset` | `%m.lottie setAnimationFromAsset %s` | `" "` |
| `lottieSetAnimationFromJson` | `%m.lottie setAnimationFromJson %s` | `" "` |
| `lottieSetAnimationFromUrl` | `%m.lottie setAnimationFromUrl %s` | `" "` |
| `lottieSetRepeatCount` | `%m.lottie setRepeatCount %d` | `" "` |
| `lottieSetSpeed` | `%m.lottie setSpeed %d` | `" "` |

**YouTubePlayerView**:

| opCode | spec | type |
|--------|------|------|
| `YTPVLifecycle` | `%m.youtubeview getLifecycle` | `" "` |
| `YTPVSetListener` | `%m.youtubeview addYouTubePlayerListener VideoID %s` | `c` |

**BadgeView**:

| opCode | spec | type |
|--------|------|------|
| `getBadgeCount` | `%m.badgeview getBadgeCount` | `d` |
| `setBadgeNumber` | `%m.badgeview setBadgeNumber %d` | `" "` |
| `setBadgeString` | `%m.badgeview setBadgeString %s` | `" "` |
| `setBadgeBackground` | `%m.badgeview setBadgeBackground %m.color` | `" "` |
| `setBadgeTextColor` | `%m.badgeview setBadgeTextColor %m.color` | `" "` |
| `setBadgeTextSize` | `%m.badgeview setBadgeTextSize %d` | `" "` |

**WaveSideBar (BubbleLayout)**:

| opCode | spec | type |
|--------|------|------|
| `setCustomLetter` | `%m.sidebar setCustomLetter String[] %s.inputOnly` | `" "` |
| `setBubbleColor` | `BubbleLayout %m.view setBubbleColor %m.color` | `" "` |
| `setBubbleStrokeColor` | `BubbleLayout %m.view setStrokeColor %m.color` | `" "` |
| `setBubbleStrokeWidth` | `BubbleLayout %m.view setStrokeWidth %d` | `" "` |
| `setBubbleCornerRadius` | `BubbleLayout %m.view setCornerRadius %d` | `" "` |
| `setBubbleArrowHeight` | `BubbleLayout %m.view setArrowHeight %d` | `" "` |
| `setBubbleArrowWidth` | `BubbleLayout %m.view setArrowWidth %d` | `" "` |
| `setBubbleArrowPosition` | `BubbleLayout %m.view setArrowPosition %d` | `" "` |

**PatternLockView**:

| opCode | spec | type |
|--------|------|------|
| `patternToString` | `%m.patternview getPattern from %m.listStr to String ` | `s` |
| `patternToMD5` | `%m.patternview getPattern from %m.listStr to MD5` | `s` |
| `patternToSha1` | `%m.patternview getPattern from %m.listStr to SHA1` | `s` |
| `patternSetDotCount` | `%m.patternview setDotCount %d ` | `" "` |
| `patternSetNormalStateColor` | `%m.patternview setNormalStateColor %m.color` | `" "` |
| `patternSetCorrectStateColor` | `%m.patternview setCorrectStateColor %m.color` | `" "` |
| `patternSetWrongStateColor` | `%m.patternview setWrongStateColor %m.color` | `" "` |
| `patternSetViewMode` | `%m.patternview setViewMode %m.patternviewmode` | `" "` |
| `patternLockClear` | `%m.patternview clearPattern` | `" "` |

**TextInputLayout**:

| opCode | spec | type |
|--------|------|------|
| `tilSetBoxBgColor` | `%m.textinputlayout setBoxBackgroundColor %m.color` | `" "` |
| `tilSetBoxStrokeColor` | `%m.textinputlayout setBoxStrokeColor %m.color` | `" "` |
| `tilSetBoxBgMode` | `%m.textinputlayout setBoxBackgroundMode %m.til_box_mode` | `" "` |
| `tilSetBoxCornerRadii` | `%m.textinputlayout setBoxCornerRadius TL %d TR %d BL %d BR %d ` | `" "` |
| `tilSetError` | `%m.textinputlayout setError %s ` | `" "` |
| `tilSetErrorEnabled` | `%m.textinputlayout setErrorEnabled %b ` | `" "` |
| `tilSetCounterEnabled` | `%m.textinputlayout setCounterEnabled %b ` | `" "` |
| `tilSetCounterMaxLength` | `%m.textinputlayout setCounterMaxLength %d ` | `" "` |
| `tilGetCounterMaxLength` | `%m.textinputlayout getCounterMaxLength` | `d` |

**InterstitialAd 扩展 (新 API)**:

| opCode | spec | type |
|--------|------|------|
| `interstitialAdLoad` | `%m.interstitialad load in %m.activity` | `" "` |
| `interstitialAdShow` | `%m.interstitialad show ad in %m.activity` | `" "` |
| `interstitialAdIsLoaded` | `%m.interstitialad is loaded` | `b` |

**ProgressDialog 扩展**:

| opCode | spec | type |
|--------|------|------|
| `progressdialogSetCanceled` | `%m.progressdialog setCanceledOnTouchOutside %b` | `" "` |

> 以上扩展块均定义在 `BlocksHandler.java` 中。部分块需要 `ConfigActivity.SETTING_SHOW_BUILT_IN_BLOCKS` 开启后才在面板中显示。
...
> 所有路径字段仅在对应文件存在时才包含，且会指向**当前实际存在**的库目录：主路径存在时写主路径；若只存在回退路径，则写入 `Android/data/<pkg>/files/local_libs/...`。
