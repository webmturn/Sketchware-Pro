...
| `onPreExecute` | — |
| `onBindCustomView` | `_position` (int), `_data` (HashMap), 各自定义视图组件引用（如 `custom_item_tv_title`） |
| `onChildMoved` | — |
...
> 以下表格由 `BlockSpecRegistry.java` + `BlocksHandler.java` + `ExtraBlocks.java` + `strings.xml` 提取生成，覆盖所有内置 opCode。
