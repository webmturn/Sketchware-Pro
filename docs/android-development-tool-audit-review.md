...
`PermissionManager` 当前已经覆盖常见运行时权限，包括旧存储权限和 Android 12/13+ 的部分新权限：
...
`ProjectFilePaths` 中蓝牙组件会添加：

- `BLUETOOTH`
- `BLUETOOTH_ADMIN`
- `BLUETOOTH_CONNECT`

但 Android 12+ 蓝牙权限模型完整场景仍需要区分：

- `BLUETOOTH_SCAN`
- `BLUETOOTH_CONNECT`
- `BLUETOOTH_ADVERTISE`

Android 13+ 媒体权限模型已经在生成层按基础场景加入：

- `READ_MEDIA_IMAGES`
- `READ_MEDIA_VIDEO`
- `READ_MEDIA_AUDIO`

当前更准确的问题是：权限生成已经补齐了部分 Android 12/13+ 权限，但仍缺少统一、完整的 targetSdk 条件化权限策略层。

风险：

- 如果未来引入蓝牙扫描或广播能力，生成项目在 Android 12+ 仍可能权限不足。
- Android 13+ 文件/媒体访问需要继续通过真实场景验证。
- 用户难以理解旧权限已经被系统弱化或忽略。
...
### 4. 内置构建依赖与导出 Gradle 依赖版本不统一（已修正）

审查时发现，`BuiltInLibraries` 中部分版本为：
...
但当时 `GradleFileGenerator` 导出依赖中仍存在：
...
当前代码已经让 `GradleFileGenerator` 复用 `BuiltInLibraries` 的版本信息生成导出 Gradle 依赖，减少了版本散落硬编码。
...
后续仍建议保留导出工程编译测试，确认导出 Gradle 依赖与手机端内置构建依赖保持行为一致。

### 5. 构建工具 asset 更新判断只比较文件大小（已修正）

审查时发现，`ProjectBuilder.hasFileChanged()` 仅通过 asset 文件大小和目标文件大小比较判断是否重新释放：
...
当前代码已经在文件大小相同时继续比较 SHA-256，避免同大小不同内容的 asset 被误判为无需更新。
...
后续仍建议观察低端设备上的构建前置耗时，因为大小相同路径会额外读取并计算较大的 asset 文件内容。
...
#### 4. 补齐 Android 13+ 媒体权限生成（已实施基础生成，建议继续验证）
...
- 已先从生成项目组件和常见文件/图片 block 侧补齐基础权限生成。
- 仍需要梳理 FilePicker、资源导入、生成项目文件访问场景。
- 不要混入 Sketchware 主 App 自身存储权限。

#### 5. 统一导出 Gradle 依赖版本表（已实施，建议持续验证）

建议：

- 保持 `GradleFileGenerator` 复用 `BuiltInLibraries` 的版本信息。
- 避免新增依赖时重新引入版本散落硬编码。
...
- 测试 FilePicker、Firebase Storage、图片文件 block、通用 FileUtil block 在 Android 13+ 的运行情况。
...
1. 修正 `requestLegacyExternalStorage` 生成条件。
2. 持续验证 Android 12+ 蓝牙权限。
3. 持续验证 Android 13+ 媒体权限。
4. 持续验证构建 asset hash 校验的性能影响。
5. 持续验证导出 Gradle 依赖与手机端构建依赖的一致性。
