...
> `ExtraPaletteBlock.java`、`StringResource.java`、`ComponentCodeGenerator.java`、`ComponentTypeMapper.java`、`ManifestGenerator.java`。
...
case "notifCreateChannel":
    opcode = String.format(
        ...
        params.get(0), params.get(0), params.get(0), params.get(1), params.get(0));
    break;
...
| `ManifestGenerator.java` | 当项目使用 Notification 组件时，自动添加 `POST_NOTIFICATIONS` 权限 | ✅ 已实现 |
...
- `POST_NOTIFICATIONS` 权限由 `ManifestGenerator` 自动写入，且 Notification 初始化/显示路径会在 Android 13+ 主动请求运行时权限

## 实际改动量

- **修改文件数**: 8 个（`ComponentTypeMapper`、`ComponentCodeGenerator`、`ManifestGenerator`、`BlockSpecRegistry`、`BlockCodeRegistry`、`ExtraPaletteBlock`、`BlockColorMapper`、`StringResource`）
- **新增代码量**: ~300 行
...
public class MainActivity extends AppCompatActivity {
    ...
    private void initialize() {
        ...
        myNotif = new NotificationCompat.Builder(getApplicationContext(), "msg_channel");
        myNotif.setSmallIcon(R.mipmap.ic_launcher);
        ...
        if (Build.VERSION.SDK_INT >= 33 && ContextCompat.checkSelfPermission(MainActivity.this, "android.permission.POST_NOTIFICATIONS") != PackageManager.PERMISSION_GRANTED) {
            ActivityCompat.requestPermissions(this, new String[]{"android.permission.POST_NOTIFICATIONS"}, 9901);
        } else {
